# @Author: Tong Zhang, Yanfang Wang
# Note this is a class project for CS61A at UC Berkeley
# Credits to 61A Staff of Fall 2015
...
class Place(object):
	...
	def remove_insect(self, insect):
		...
		"""
		if insect.name != 'Queen' or insect.imposter:
			if insect.is_ant:
...
class Ant(Insect):
	...
	def can_contain(self,other_ant):
		return (self.container and self.ant == None and not other_ant.container)
...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		"""
		# BEGIN Problem 3B
		place_maybe = self.place
		while place_maybe.bees == [] and place_maybe != hive:
			place_maybe = place_maybe.entrance
		if place_maybe != hive:
			return random_or_none(place_maybe.bees)
		else:
			return None
...
class Water(Place):
	...
	def add_insect(self, insect):
		...
		# BEGIN Problem 3A
		Place.add_insect(self,insect)   # why not self.add_insect(insect)?
...
class FireAnt(Ant):
	...
	def reduce_armor(self, amount):
		...
		if self.armor <=0:
			...
			[i.reduce_armor(self.damage) for i in list_of_bees]
			Ant.reduce_armor(self,0)
			
	### Do we need to have action for every specific type of ant???
		# END Problem 4A
...
class LongThrower(ThrowerAnt):
	...
	def nearest_bee(self,hive):
		...
		for i in range(0,self.min_range):
			...
			ant1 = Ant()
			starting_place.add_insect(ant1)
		return ThrowerAnt.nearest_bee(ant1,hive)
		starting_place.remove_insect(ant1)
...
class ShortThrower(ThrowerAnt):
	...
	def nearest_bee(self,hive):
		new_hive = self.place
		for i in range(0,self.max_range):
			new_hive = new_hive.entrance
		return ThrowerAnt.nearest_bee(self,new_hive)
...
class HungryAnt(Ant):
	...
	def action(self, colony):
		# BEGIN Problem 6B
		if self.digesting == 0:
			caught_bee = random_or_none(self.place.bees)
			if caught_bee != None:
				self.eat_bee(caught_bee)
		else:
			self.digesting -= 1
		# END Problem 6B
...
class TankAnt(BodyguardAnt):
	...
	def action(self, colony):
		# BEGIN Problem 8
		list_of_bees = self.place.bees[:]
		[i.reduce_armor(self.damage) for i in list_of_bees]  ## Why would happen?
...
class QueenAnt(ScubaThrower):  # You should change this line
	...
	def action(self, colony):
		...
		if self.imposter:
			...
			empower_position = self.place.exit
			while empower_position is not None:
				if empower_position.ant is not None:
					if empower_position.ant not in self.behind_list:
						empower_position.ant.damage = 2 * empower_position.ant.damage
						self.behind_list.append(empower_position.ant)
						if empower_position.ant.name in ['Bodyguard','Tank'] and empower_position.ant.ant != None and empower_position.ant.ant not in self.behind_list:
							empower_position.ant.ant.damage = 2* empower_position.ant.ant.damage
							self.behind_list.append(empower_position.ant.ant)
					else:
						if empower_position.ant.name in ['Bodyguard','Tank'] and empower_position.ant.ant != None and empower_position.ant.ant not in self.behind_list:
							empower_position.ant.ant.damage = 2* empower_position.ant.ant.damage
							self.behind_list.append(empower_position.ant.ant)
					# if empower_position.ant not in self.behind_list and empower_position.ant.name != 'Bodyguard':
					#     empower_position.ant.damage = 2 * empower_position.ant.damage
					#     self.behind_list.append(empower_position.ant)
					# if empower_position.ant not in self.behind_list and 
					#     if empower_position.ant.name == 'Bodyguard':
					# 	    empower_position.ant.ant.damage = 2* empower_position.ant.ant.damage
				empower_position = empower_position.exit
...
def make_stun(action):
	...
	##Nonlocal parameter equal to duration
	# END Problem EC
...
def apply_effect(effect, bee, duration):
	...
	def another(colony):
		...
		if duration >0:
			duration -= 1
			return effect(original_mehod)(colony)
		else:
			return original_mehod(colony)
	original_mehod =bee.action
	bee.action = another
